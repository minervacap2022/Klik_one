...
class GrowthRepositoryImpl(
  ...
  override suspend fun getAchievements(): Result<Achievements> = try {
    val achievements = dataSource.getAchievements()
      ?: throw IllegalStateException("Achievements data not loaded from backend")
  ...
  override suspend fun getScenarios(): Result<List<Scenario>> = try {
  ...
  override suspend fun getGrowthHistory(days: Int): Result<List<GrowthDataPoint>> = try {
  ...
  override suspend fun getNetworkGrowthStats(): Result<NetworkGrowthStats> = try {
  ...
  override suspend fun getEngagementStats(): Result<EngagementStats> = try {
  ...
  override suspend fun refreshAchievements(): Result<Unit> = try {
    refreshAchievementsInternal()
    Result.Success(Unit)
  } catch (e: Exception) {
