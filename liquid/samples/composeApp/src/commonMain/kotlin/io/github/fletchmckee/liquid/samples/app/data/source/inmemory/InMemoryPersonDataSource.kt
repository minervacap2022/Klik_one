...
class InMemoryPersonDataSource {
    ...
    fun getPeopleByTier(tier: InfluenceTier): List<Person> {
        return people.filter { it.influenceTier == tier }
    }

    fun getPeopleByRelationshipStatus(status: RelationshipStatus): List<Person> {
        return people.filter { it.relationshipStatus == status }
    }

    fun getPeopleByOrganization(organization: String): List<Person> {
        return people.filter {
    ...
    fun getPeopleForMeeting(meetingId: String): List<Person> {
        return meetingParticipants[meetingId] ?: emptyList()
    }

    fun getPeopleForProject(projectId: String): List<Person> {
        return people.filter { it.relatedProjects.contains(projectId) }
    }

    fun getTopInfluencers(limit: Int): List<Person> {
        return people
            .filter { it.influenceTier == InfluenceTier.S }
            .take(limit)
    }

    fun getStrongRelationships(): List<Person> {
        return people.filter { it.relationshipStatus == RelationshipStatus.STRONG }
    }
    ...
    fun getPinnedPeople(): List<Person> {
        return people.filter { it.isPinned && !it.isArchived }
