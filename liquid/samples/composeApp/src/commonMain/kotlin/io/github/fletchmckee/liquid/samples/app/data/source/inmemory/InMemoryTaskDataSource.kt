...
class InMemoryTaskDataSource {
  ...
  fun getTasksByStatus(status: TaskStatus): List<TaskMetadata> = tasks.filter { it.status == status && !it.isArchived }

  fun getTasksByPriority(priority: TaskPriority): List<TaskMetadata> = tasks.filter { it.priority == priority && !it.isArchived }

  fun getPinnedTasks(): List<TaskMetadata> = tasks.filter { it.isPinned && !it.isArchived }
  ...
  fun deleteTask(taskId: String): Boolean = tasks.removeAll { it.id == taskId }
  ...
  fun getTodoItems(taskId: String): List<TodoItem> = todoItems[taskId]?.toList() ?: emptyList()
  ...
  fun getTasksForMeeting(meetingId: String): List<TaskMetadata> = tasks.filter { it.relatedMeetingId == meetingId }

  fun getTasksForProject(projectId: String): List<TaskMetadata> = tasks.filter {
