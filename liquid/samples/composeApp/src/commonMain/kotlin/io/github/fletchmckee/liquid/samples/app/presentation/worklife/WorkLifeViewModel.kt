...
class WorkLifeViewModel : BaseViewModel<WorkLifeUiState, WorkLifeEvent>() {
   ...
   */
  fun getSortedOrganizations(): List<Organization> = currentState.organizations.sortedWith(
   ...
   */
  fun getSortedProjects(): List<Project> = currentState.projects.sortedWith(
   ...
   */
  fun getPeopleByTier(): Map<InfluenceTier, List<Person>> = getFilteredPeople().groupBy { it.influenceTier }

  /**
   * Get people grouped by related organizations
   */
  fun getPeopleByOrganization(): Map<String, List<Person>> = getFilteredPeople()
   ...
   */
  fun getPeopleNeedingAttention(): List<Person> = currentState.people.filter {
    it.relationshipStatus == RelationshipStatus.DEVELOPING
  }
   ...
   */
  fun getPeopleStats(): PeopleStats = PeopleStats(
