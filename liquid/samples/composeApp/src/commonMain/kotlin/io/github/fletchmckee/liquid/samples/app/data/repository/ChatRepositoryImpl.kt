...
class ChatRepositoryImpl : ChatRepository {
    ...
    private fun loadPersistedHistory() {
        try {
            ...
            _messages.clear()
            _messages.addAll(history.messages.map { persisted ->
                ChatMessage(
                    id = persisted.id,
                    text = persisted.text,
    ...
    override suspend fun getChatMessages(): Result<List<ChatMessage>> {
        return Result.Success(_messages.toList())
    }
    ...
    override suspend fun getContextualSuggestions(context: ChatContext): Result<List<SuggestedQuestion>> {
    ...
    override suspend fun getChatHistory(startTime: Long, endTime: Long): Result<List<ChatMessage>> {
        return try {
    ...
    override suspend fun exportChatHistory(): Result<String> {
        return try {
