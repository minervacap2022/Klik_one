...
class AskKlikViewModel : BaseViewModel<AskKlikUiState, AskKlikEvent>() {
  ...
  fun sendMessage() {
    ...
    launch {
      ...
      when (val result = sendChatMessageUseCase(message)) {
        ...
        is Result.Error -> {
          val msg = result.message ?: "Failed to send message"
          updateState {
            copy(
              isSending = false,
              currentInput = message, // Restore input on error so user can retry
              error = msg,
            )
          }
          sendEvent(AskKlikEvent.ShowError(msg))
        }
   ...
   */
  fun getLastUserMessage(): ChatMessage? = currentState.messages.lastOrNull { it.isUser }

  /**
   * Get the last AI response
   */
  fun getLastAiResponse(): ChatMessage? = currentState.messages.lastOrNull { !it.isUser }

  /**
   * Check if there are any messages
   */
  fun hasMessages(): Boolean = currentState.messages.isNotEmpty()
