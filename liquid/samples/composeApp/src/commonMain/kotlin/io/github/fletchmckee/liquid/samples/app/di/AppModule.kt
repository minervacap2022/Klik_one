...
object AppModule {
   ...
   */
  suspend fun archiveMeeting(meetingId: String): io.github.fletchmckee.liquid.samples.app.core.Result<Unit> = calendarRepository.archiveMeeting(meetingId)

  /**
   * Archive a project by ID.
   * This sets isArchived = true and removes it from the visible list.
   */
  fun archiveProject(projectId: String): Boolean = projectDataSource.archiveProject(projectId)

  /**
   * Archive a person by ID.
   * This sets isArchived = true and removes it from the visible list.
   */
  fun archivePerson(personId: String): Boolean = personDataSource.archivePerson(personId)

  /**
   * Archive an organization by ID.
   * This sets isArchived = true and removes it from the visible list.
   */
  fun archiveOrganization(organizationId: String): Boolean = organizationDataSource.archiveOrganization(organizationId)

  /**
   * Unarchive a project by ID.
   * This sets isArchived = false and restores it to the visible list.
   */
  fun unarchiveProject(projectId: String): Boolean = projectDataSource.unarchiveProject(projectId)

  /**
   * Unarchive a person by ID.
   * This sets isArchived = false and restores it to the visible list.
   */
  fun unarchivePerson(personId: String): Boolean = personDataSource.unarchivePerson(personId)

  /**
   * Unarchive an organization by ID.
   * This sets isArchived = false and restores it to the visible list.
   */
  fun unarchiveOrganization(organizationId: String): Boolean = organizationDataSource.unarchiveOrganization(organizationId)
   ...
   */
  fun getInitialPreferences(): io.github.fletchmckee.liquid.samples.app.domain.entity.UserPreferences? = userDataSource.getUserPreferences()
}
...
private fun ModelTaskMetadata.toDomainTaskMetadata(): DomainTaskMetadata = DomainTaskMetadata(
  id = id,
  title = title,
  subtitle = subtitle,
  context = context,
