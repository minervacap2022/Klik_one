...
class InMemoryProjectDataSource {
  ...
  fun getProjectsByStatus(status: ProjectStatus): List<Project> = projects.filter { it.status == status }

  fun getActiveProjects(): List<Project> = projects.filter { it.status == ProjectStatus.ON_TRACK }
  ...
  fun getProjectsForPerson(personId: String): List<Project> = projects.filter { project ->
  ...
  fun getProjectsForOrganization(organizationId: String): List<Project> = projects.filter { project ->
  ...
  fun deleteProject(projectId: String): Boolean = projects.removeAll { it.id == projectId }

  fun getProjectStatistics(): ProjectStatistics = ProjectStatistics(
  ...
  fun getPinnedProjects(): List<Project> = projects.filter { it.isPinned && !it.isArchived }
