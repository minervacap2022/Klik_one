...
class InMemoryChatDataSource {
   ...
   */
  private fun generateDefaultResponse(): ChatMessage = ChatMessage(
  ...
  fun getSuggestedQuestions(): List<SuggestedQuestion> = suggestedQuestions.toList()
  ...
  fun deleteMessage(messageId: String): Boolean = chatMessages.removeAll { it.id == messageId }

  fun getChatHistory(startTime: Long, endTime: Long): List<ChatMessage> = chatMessages.filter { it.timestamp in startTime..endTime }

  fun exportChatHistory(): String = chatMessages.joinToString("\n\n") { msg ->
