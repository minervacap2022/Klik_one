...
class InMemoryCalendarDataSource {
    ...
    fun getMeetingsForDate(date: LocalDate): List<Meeting> {
        return meetings.filter { it.date == date && !it.isArchived }
    }

    fun getMeetingsInRange(startDate: LocalDate, endDate: LocalDate): List<Meeting> {
        return meetings.filter { it.date in startDate..endDate && !it.isArchived }
    }
    ...
    fun getPinnedMeetings(): List<Meeting> {
        return meetings.filter { it.isPinned }.sortedByDescending { it.pinnedAt }
    }
    ...
    fun getMeetingMinutes(meetingId: String): List<MeetingMinute> {
        return getMeetingById(meetingId)?.minutes ?: emptyList()
    }
    ...
    fun getDailyBriefing(): DailyBriefing {
        return dailyBriefing ?: throw IllegalStateException(
            "No daily briefing available. Backend must provide daily briefing data."
        )
    }

    fun getUpcomingMeetingsCount(): Int {
        return meetings.count { it.date >= today && !it.isArchived }
    }
}
