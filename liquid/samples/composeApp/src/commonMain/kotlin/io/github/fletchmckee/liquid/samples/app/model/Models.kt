 ...
 */
fun filterArchivedTasks(tasks: List<TaskMetadata>): List<TaskMetadata> {
    return tasks.filter { it.id !in archivedTaskIdsState.value }
}

/**
 * Get non-archived meetings from a list
 */
fun filterArchivedMeetings(meetings: List<Meeting>): List<Meeting> {
    return meetings.filter { it.id !in archivedMeetingIdsState.value }
}
...
fun getMeetingsForDate(meetings: List<Meeting>, date: LocalDate): List<Meeting> {
    return meetings.filter { it.date == date }
}
...
fun getDayOfWeek(date: LocalDate): Int {
    return getFirstDayOfWeek(date.year, date.monthNumber) + date.dayOfMonth - 1
}
